...
# Some child interpreters used by the inherited matrix can still leave
# transient bytecode behind. Remove only generated bytecode under the checked
# source trees before the strict privacy/inventory gate runs.
find "$PROJECT_ROOT/scripts" "$SOURCE_PLUGIN_ROOT" -type f \
  \( -name '*.pyc' -o -name '*.pyo' \) -delete
find "$PROJECT_ROOT/scripts" "$SOURCE_PLUGIN_ROOT" -type d -name __pycache__ -empty -delete
