...
def _controlled_term_pattern(term: str) -> re.Pattern[str]:
    term_pattern = r"[\W_]*".join(re.escape(character) for character in term)
    return re.compile(rf"(?<![a-z0-9]){term_pattern}(?![a-z0-9])")


_CONTROLLED_TERM_PATTERNS = tuple(
    (term, _controlled_term_pattern(term))
