...
Use this versioned branch only when the root router supplies the complete composite. Capture it once through `build_validated_private_vacancy_application_packet_v1`; that boundary recomputes `recommended_next_action=prepare_private_vacancy_packet`, builds the packet, and returns its opaque validated snapshot. Eligibility remains the sole vacancy and trigger authority; accept no independent selector or caller-supplied packet JSON. Read [candidate-fact-matrix-v1.schema.json](../../schemas/candidate-fact-matrix-v1.schema.json), [private-vacancy-application-packet-v1.schema.json](../../schemas/private-vacancy-application-packet-v1.schema.json), and the exact workflow in [asset-workflow.md](references/asset-workflow.md). Do not hand-author the retired identity-bearing prose packet.
...
Execution proof is the private packet JSON, rendered HTML, and exact receipts emitted by passing that same opaque validated snapshot to both writers. On success, return only the root skill's four-section identity-free artifact delivery. Do not emit the ordinary asset response, candidate or provenance IDs, router fields, a `module_execution_packet`, source bindings, or receipt JSON.
