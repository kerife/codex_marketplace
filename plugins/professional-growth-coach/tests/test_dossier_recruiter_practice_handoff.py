...
class DossierRecruiterPracticeHandoffTests(unittest.TestCase):
    ...
    def test_builder_accepts_validated_v2_projection_and_binds_original_snapshot(self):
        dossier = self._v2_dossier()
        source_snapshot = handoff_builder.snapshot_for_dossier(dossier)
        validator = handoff_builder._load_sibling("validate_executive_career_dossier_v2")
        ...
        self.assertEqual(
            self.fixture["expected"]["dossier_projection"], handoff["dossier_projection"]
        )

    def test_validator_accepts_v2_source_with_original_snapshot_and_v1_practice_projection(self):
        dossier = self._v2_dossier()
        source_snapshot = handoff_builder.snapshot_for_dossier(dossier)
        handoff = build_handoff(dossier, self.fixture["vacancy"], source_snapshot)
        practice_path = (
    ...
    def test_v2_source_snapshot_fails_closed_after_ledger_priority_or_section_mutation(self):
        dossier = self._v2_dossier()
        source_snapshot = handoff_builder.snapshot_for_dossier(dossier)
        mutations = (
            lambda value: value["section_coverage"][2].update({
            ...
            lambda value: value["priorities"][0].update({
                "action": "Revisar el enfoque con una plantilla privada distinta.",
            }),
            lambda value: value["evidence"][0].update({"profile_section": "about"}),
        )
        ...
        for mutate in mutations:
            with self.subTest(mutate=mutate):
                ...
                mutate(changed)
                with self.assertRaisesRegex(ValueError, "dossier validation failed|source_snapshot"):
