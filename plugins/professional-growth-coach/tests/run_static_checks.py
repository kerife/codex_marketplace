...
PRIVATE_VACANCY_APPLICATION_PACKET_RELEASE_PATHS = (
    ...
    "schemas/learning-proof-sprint-v1.schema.json",
    "scripts/build_learning_proof_sprint_v1.py",
    "scripts/validate_learning_proof_sprint_v1.py",
    "scripts/write_learning_proof_sprint_v1.py",
    "scripts/render_learning_proof_sprint_v1.py",
    "scripts/learning_proof_sprint_identity.py",
    "assets/learning-proof-sprint-v1.html",
    "assets/learning-proof-sprint-v1.css",
    "tests/test_learning_proof_sprint_v1.py",
    "tests/test_render_learning_proof_sprint_v1.py",
...
def validate_executive_dossier_package(
    ...
    for relative_path in EXECUTIVE_DOSSIER_V2_PACKAGE_PATHS:
        ...
        if _package_path_traverses_symlink(root, relative_path):
            errors.append(f"{relative_path}: dossier v2 package path cannot traverse a symlink")
    ...
    if v2_schema_relative in v2_safe_paths:
        try:
            v2_schema = json.loads((plugin_root / v2_schema_relative).read_text(encoding="utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError):
    ...
    for fixture_relative in EXECUTIVE_DOSSIER_V2_PACKAGE_PATHS[-2:]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temporary_directory:
                ...
                validator_result = subprocess.run(
                    [sys.executable, "-B", str(plugin_root / "scripts/validate_executive_career_dossier_v2.py"), str(input_path)],
                    cwd=runtime_root, capture_output=True, text=True, check=False, timeout=20,
                )
                render_result = subprocess.run(
                    [sys.executable, "-B", str(plugin_root / "scripts/render_executive_career_dossier_v2.py"), str(input_path), "--output", str(output_path)],
                    cwd=runtime_root, capture_output=True, text=True, check=False, timeout=20,
                )
                output_mode = stat.S_IMODE(output_path.stat().st_mode) if output_path.is_file() else None
        ...
        if render_result.returncode != 0 or output_mode != 0o600:
    ...
    for learning_name, research_name, dossier_name, provider_name in learning_cases:
        try:
            ...
            requests = [
                ...
                for row in expected_learning["decisions"]
            ]
            built_learning = learning_v2_builder.build_learning_bundle_v2(
                research, market, dossier, provider, requests
            )
            learning_errors = learning_v2_validator.validate_learning_bundle_v2(
                expected_learning, research, market, dossier, provider
            )
        except Exception:
            errors.append(f"{learning_name}: learning v2 fixture composition failed")
            continue
    ...
    if case_id == "unsupported-technology-confirmation":
        ready_copy_text = "\n".join(
            ...
            for card in re.findall(
                r'<article\b[^>]*\bclass="card copy-card span-4"[^>]*>(.*?)</article>',
                html_text,
...
def _inventory_relative_path(root: Path, path: Path) -> str:
    try:
...
def _validate_bytecode_inventory(root: Path, scope: str) -> list[str]:
    errors: list[str] = []
    if not root.is_dir():
        return [f"{scope} root is missing"]
    for path in sorted(root.rglob("*")):
        relative = _inventory_relative_path(root, path)
        if path.is_dir() and path.name == "__pycache__":
            errors.append(f"{relative}: {scope} forbids bytecode")
        elif path.is_file() and path.suffix.lower() in {".pyc", ".pyo"}:
            errors.append(f"{relative}: {scope} forbids bytecode")
    return errors


def validate_release_inventory(plugin_root: Path) -> list[str]:
    """Reject generated bytecode from source and release trees."""

    errors = _validate_bytecode_inventory(plugin_root, "release inventory")
    if not plugin_root.is_dir():
        return errors
    for path in sorted(plugin_root.rglob("*")):
        if not path.is_file():
            continue
        metadata = _contains_personal_metadata(path)
        if metadata is None:
            errors.append("release inventory contains unreadable artifact")
        elif metadata:
            relative = _inventory_relative_path(plugin_root, path)
            errors.append(f"{relative}: release inventory contains personal metadata")
    return errors


def _contains_personal_metadata(path: Path) -> bool | None:
    try:
        payload = path.read_bytes()
    except OSError:
        return None
    return any(pattern.search(payload) for pattern in _PERSONAL_METADATA_PATTERNS)


def validate_installed_cache_inventory(cache_root: Path) -> list[str]:
    """Verify installed artifacts contain no bytecode or local provenance paths."""

    errors = _validate_bytecode_inventory(cache_root, "installed cache")
    ...
    for path in sorted(cache_root.rglob("*")):
        ...
        metadata = _contains_personal_metadata(path)
        if metadata is None:
            errors.append("installed cache contains unreadable artifact")
        elif metadata:
            relative = _inventory_relative_path(cache_root, path)
            errors.append(f"{relative}: installed cache contains personal metadata")
    return errors
