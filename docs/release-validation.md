...
rule ID, and count. Before that strict final scan, the release runner removes
only transient `.pyc`/`.pyo` files and empty `__pycache__` directories under
`scripts/` and the plugin source tree; generated bytecode is never accepted
into the release inventory.
