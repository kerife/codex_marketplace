...
active_plugin="$CODEX_HOME/plugins/cache/job-search-coach-local/job-search-coach/$active_version"
...
```bash
python3 -B "$CODEX_HOME/skills/.system/plugin-creator/scripts/update_plugin_cachebuster.py" plugins/job-search-coach
```
...
diff -qr plugins/job-search-coach "$CODEX_HOME/plugins/cache/job-search-coach-local/job-search-coach/$installed_version"
git diff --check
